...
/* creates a new empty sorted list, returns it on allocation success,
    or NULL on failure */
/* compare : returns a positive integer if 'd1' should be after 'd2', 0 if their
    order is the same, and a negative number if 'd1' should be before 'd2' */
/* O(n) */
...
sr_iter_t SortedLFind(sr_iter_t from, sr_iter_t to, const void *data, sr_list_t *srlist);
/*----------------------------------------------------------*/
/* Removes the given iterator of srlist and returns the next iterator */
